...
exports.getUserPlatformStores = async (req, res) => {
    try {
      const userId = req.user.id;
      
      // 1. 사용자에게 할당된 스토어 코드 조회
      ...
      if (assignmentError) {
        console.error('스토어 할당 조회 오류:', assignmentError);
        return res.status(500).json({ error: '스토어 할당 정보를 가져오는데 실패했습니다.' });
      }
      
      if (!assignments || assignments.length === 0) {
        return res.status(200).json([]);  // 할당된 스토어가 없는 경우 빈 배열 반환
      }
      
      // 스토어 코드 추출
      const storeCodes = assignments.map(a => a.store_code);
      
      // 2. 할당된 스토어의 플랫폼 규칙 조회 (플랫폼별 정보 포함)
      ...
      if (platformsError) {
        console.error('플랫폼 규칙 조회 오류:', platformsError);
        return res.status(500).json({ error: '플랫폼 정보를 가져오는데 실패했습니다.' });
      }
      
      // 플랫폼 정보가 없는 경우, 기본 스토어 정보만 반환
      if (!platforms || platforms.length === 0) {
        ...
        if (storesError) {
          console.error('스토어 정보 조회 오류:', storesError);
        ...
        const result = stores.map(store => ({
          ...
          store_name: store.store_code, // 이름 정보가 없는 경우 코드로 대체
          platform: '배달의민족',  // 기본 플랫폼으로 설정
          platform_code: ''
        }));
      ...
      console.error('사용자 플랫폼 스토어 조회 오류:', error);
